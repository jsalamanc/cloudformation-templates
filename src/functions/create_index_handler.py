...
def wait_for_opensearch_ready(client, max_attempts=30):
    """
    ...
    for attempt in range(max_attempts):
        try:
            ...
            # Reintentar para errores de autorización y conexión
            if any(keyword in error_msg.lower() for keyword in ["403", "forbidden", "authorization", "404", "notfounderror", "connection refused", "timeout", "unavailable", "service unavailable", "host is not available"]):
                if attempt < max_attempts - 1:
                    delay = min(base_delay * (1.5 ** attempt), max_delay)
                    jitter = random.uniform(0.8, 1.2) * delay
                    actual_delay = delay + jitter
                    
                    print(f"OpenSearch/permisos no están listos. Esperando {actual_delay:.1f} segundos antes del siguiente intento...")
